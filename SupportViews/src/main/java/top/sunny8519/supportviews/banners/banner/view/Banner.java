package top.sunny8519.supportviews.banners.banner.view;
...
import top.sunny8519.supportviews.R;
import top.sunny8519.supportviews.banners.banner.BannerConfig;
import top.sunny8519.supportviews.banners.banner.BannerScroller;
import top.sunny8519.supportviews.banners.banner.WeakHandler;
import top.sunny8519.supportviews.banners.banner.displayer.ViewDisplayer;
import top.sunny8519.supportviews.banners.banner.listener.OnBannerClickListener;
...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    private int mIndicatorSelectedResId = R.drawable.shape_banner_indicator_white_bg;
    private int mIndicatorUnselectedResId = R.drawable.shape_banner_indicator_gray_bg;
    ...
    private void handleTypedArray(Context context, AttributeSet attrs) {
        ...
        mIndicatorSelectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_selected, R.drawable.shape_banner_indicator_white_bg);
        mIndicatorUnselectedResId = typedArray.getResourceId(R.styleable.Banner_indicator_drawable_unselected, R.drawable.shape_banner_indicator_gray_bg);
        ...
        titleTextColor = typedArray.getColor(R.styleable.Banner_title_text_color, BannerConfig.TITLE_TEXT_COLOR);
        titleTextSize = typedArray.getDimensionPixelSize(R.styleable.Banner_title_text_size, BannerConfig.TITLE_TEXT_SIZE);
    ...
    public Banner setViewDisplayer(ViewDisplayer viewDisplayer) {
    ...
    private void setImageList(List<?> imagesUrl) {
        ...
        initImages();
        // 创建count+2个View，实现View的循环轮播
        for (int i = 0; i <= count + 1; i++) {
            View itemView = null;
            if (viewDisplayer != null) {
                // 存在ImageLoaderInterface对象则使用该对象创建View
                itemView = viewDisplayer.createView(context);
            }
            if (itemView == null) {
                itemView = new ImageView(context);
            }
            // 如果itemView是ImageView，设置ImageView的scaleType类型
            setScaleType(itemView);
            Object url;
            if (i == 0) {
            ...
            if (viewDisplayer != null)
                viewDisplayer.displayImage(context, url, itemView);
            else
...
//        Log.i(tag,"currentItem: "+currentItem);
        Log.d("sunny", "currentItem = " + currentItem + " state = " + state);
        switch (state) {
            case 0://No operation
                if (currentItem == 0) {
